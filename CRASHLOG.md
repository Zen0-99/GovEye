...
## Bug: Voting/finance/activity data disappears on MP profile revisit
**Date:** 2026-09-05
**Symptom:** Navigating to an MP profile from Directory > Officials showed all data (votes, finances, activity). But navigating to the same MP from Party > Members showed empty Stats/Finances/Activity tabs. Returning to Directory > Officials > same MP still showed empty data. Only restarting the app fixed it.
**Root cause:** `ProfileCache` (in-memory LRU cache for instant MP profile revisits) only stored header fields (mp, bioData, activityScore, traitBars, mpTags, etc.) but NOT `memberVotes`, `rebellionStats`, `interests`, `expenses`, `committees`, `experiences`, `allDivisionDates`. On any revisit within the 5-minute TTL, `loadProfile()` restored only the cached header fields and called `refreshFromDb()` — but `refreshFromDb()` also only reloaded header fields, leaving votes/interests/expenses/committees/experiences at their default empty values. The `loadActivityFeed()` method then reused the empty `state.memberVotes` and `state.interests`, producing an empty activity feed. This made it look like all voting/finance/activity data had disappeared, when in fact the DB had all the data (2.1M votes, 650 mp_stats rows, 45K interests).
**Fix:** (1) Added all missing fields (`memberVotes`, `rebellionStats`, `allDivisionDates`, `allVotesByDivision`, `memberPartyName`, `interests`, `expenseBucketTotals`, `expenses`, `committees`, `experiences`) to `CachedProfileData`. (2) Restored all fields on cache hit in `loadProfile()`. (3) Stored all fields in the cache at the end of `loadProfile()`. (4) Updated `refreshFromDb()` to reload all missing fields (votes, interests, expenses, committees, experiences) in parallel, matching the Stage 2 loading in `loadProfile()`.
**Files:** `ProfileCache.kt`, `MpProfileViewModel.kt`
