 ...
 */
data class FilterOptionData(val label: String, val isSelected: Boolean, val onClick: () -> Unit)

/**
 * A filter section (e.g., "Party", "House", "Status").
 * Adapted from Miko's FilterSectionData — simplified (no LibraryFilterId).
 */
data class FilterSectionData(val id: String, val title: String, val items: List<FilterOptionData>)
...
data class DirectoryFilterState(
     ...
     */
    fun matches(mp: Mp): Boolean = (houseFilter == 0 || mp.house == houseFilter) &&
